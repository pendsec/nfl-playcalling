"""Off-policy evaluation (Step 5): value any candidate policy from logged data.
    ...
    doubly_robust  — V_DR(pi): the direct method plus an importance-weighted
                     ...
                     estimator every policy claim is judged by.
...
A standalone IPW estimator is deliberately absent (see CLAUDE.md Step 5): DR's
correction term already does the reweighting, and at this slice's ESS ratio a
pure-IPW figure would be dominated by a few plays on the propensity floor
without changing any decision. It becomes worth adding once multi-team pooling
lifts the effective sample size.
"""
